...
Digital transformation have the potential to make inclusive health systems in low- and middle income countries (LMICs) a reality by connecting patients and professionals, using data as a currency of trust. Digital technologies have the potential to increase the availability, accessibility, acceptability, and quality of health services as we know them; change nature and direction of health care, and the possibility of making it more preventive, personalised, and mobile through the use of such technologies; and enfranchise patients and communities (and particularly vulnerable groups including children and young people) and evolve their relationship with health professionals and providers, thus helping shape the health system according to the needs of the patients and communities [@kickbusch2021lancet].
...
To achieve scaleability of digital health interventions, we need to design and implement standardized open health data systems (OHDS) and their associated ecosystems that can support the wider health sector in improving along five dimensions, namely [@kelley2020digital]:
...
Learnings from digital health interventions have shown that improvements along the five aforementioned dimensions is indeed within reach. As an example, in our experience with the MomCare programme, we have demonstrated that routinely collected health data, combined with financial data, can be effectively used to gain insight into the continuity of care, improve clinical adherence whilst maintaining efficiency and affordability of health services in LMICs. By actively coaxing pregnant mothers to undergo 4 antenatal check-ups, outcomes were improved whilst maintaining the average cost of MCH services [@huisman2022digital;@sanctis2022maintaining;@izudi2023experiences].

MomCare, like most other digital health interventions, is critically dependent on the availability of data and analytics functions within the OHDS. For example, the widely used DHIS2 reports that are most commonly used in LMICs contained insufficient information to run the programme. More detailed analytics at the patient level were required. It is, in fact, a widely held belief that data & analytics are an essential part of OHDSs. This is exemplified how the OpenHIE reference architecture [@openhie] has been adopted by many sub-Saharan African countries as the blueprint for implementing nation-wide health information exchanges (HIE) [@mamuye2022health], including Nigeria [@dalhatu2023paper], Kenya [@mbugua2021adoption] and Tanzania [@nsaghurwe2021one]. These countries have, as a matter of course, extended the framework to include “data & analytics services” as an additional domain [@mbugua2021adoption;@dalhatu2023paper]. The rationale for this addition is to facilitate secondary reuse of health data for academic research, real-world evidence studies etc. which can be framed within the context of ongoing efforts towards Findable, Accessible, Interoperable and Reusable (FAIR) sharing of health data [@guillot2023fair].


## Extending OpenHIE to include modern data and analytics standards

If we intend to use the OpenHIE standard not only for primary health data sharing, but also to support data and analytics for secondary health data sharing, we need to extend its specification. The main contribution of this paper is to propose how recent standards and open source implementations from the data engineering community could be integrated into the OpenHIE framework, as illustrated in @fig-ohie.
...
+-------------------+---------------------------------------------------------------------------------------+
: Adaptation of lakehouse zones for healthcare specific standardization of Data and Analysis Services {#tbl-data-and-analysis-services}

[TO DO: need to explain table (is too condensed now) and especially FHIR specific components.]

In the remainder of this paper, we first describe in more detail how the FHIR lakehouse pattern can be implemented using existing open source technologies. We do so by presenting work done at PharmAccess. Code and digital artifacts are made available [TO DO: include links to support GitHub repositories].

Subquently, we compare this solution design with two widely used OpenHIE-compliant frameworks, namely the OpenHIM platform ([https://jembi.gitbook.io/openhim-platform/](https://jembi.gitbook.io/openhim-platform/)) and the OnaData platform [https://ona.io/home/products/ona-data/features/](https://ona.io/home/products/ona-data/features/). We take a narrative approach in presenting our design and the case studies, surveying existing scientific studies on OHDSs, focusing on the seminal reports and subsequently searching forward citations. In addition, we have searched the open source repositories (most notably GitHub) and the online communities (OpenHIE community, FHIR community) to search for relevant open standards, technologies and architectures. This paper should not be considered as a proper systematic review.

Finally, we discuss our findings and propose routes for future development.
