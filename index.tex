...
Extract-Load-Transform pattern (ELT) \citep{hai2023data} where data is
...
Substrait for Intermediate Representation (standard A) and Apache Arrow
(standard B and C). The composable data stack can be implemented with
current open source technologies (Figure~\ref{fig-cds-examples}). As an
example, the Ibis user interface is currently sufficiently mature to
offer a standardized dataframe interface to 19 different execution
engines.
